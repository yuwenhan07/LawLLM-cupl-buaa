...
def process_func(example):
    ...
    # 构建指令和输入文本的序列
    instruction_text = (
        f"<|system|>\n请你根据下面提供的'法律文本材料'内容，回答相应的'问题'，以完成片段抽取式的阅读理解任务。\n具体来说，你要正确回答'问题'，并且答案限定是'法律文本材料'的一个子句（或片段）。请你以'''答案：A'''的格式给出回答，其中A表示'法律文本材料'中正确的子句（或片段）。<|endoftext|>\n<|user|>\n{example['input']}<|endoftext|>\n<|assistant|>\n"
    )
...
# 加载数据集
input_path = "../data/legal-cu-files/test.jsonl"
train_dataset_path = "../data/legal-cu-files/LRC_train_origin.jsonl"
test_dataset_path = "../data/legal-cu-files/LRC_test_origin.jsonl"
split_jsonl_file(input_path, train_dataset_path, test_dataset_path)

train_jsonl_new_path = "../data/legal-cu-files/LRC_train.jsonl"
test_jsonl_new_path = "../data/legal-cu-files/LRC_test.jsonl"
...
swanlab_callback = SwanLabCallback(
    project="GLM4-LRC-fintune",
    experiment_name="GLM4-9B-Chat",
    description="使用智谱GLM4-9B-Chat模型在法律文本阅读理解。",
    config={
        "model": "GLM4-9B-Chat",
        "dataset": "法律文本阅读理解",
    },
