...
for query_idx, query in enumerate(queries):
    ...
    for i in range(1, 6):
        ...
        if question_key in query:
            question = query[question_key]
            combined_query += f" {question}"
    
    print(f"Processing query {query_idx + 1}: {combined_query}")
    query_results = search(combined_query)
...
# 将结果写入JSON文件
results_path = '../data/search_results_with_question.json'
